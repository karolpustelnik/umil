...
class CreateLimbHeatmaps:
    def __init__(self, json_root, img_w=224, blur_factor=224, limbs=None, squish_channels=True, use_heatmap = True):
        if squish_channels is None:
            squish_channels = False

        if limbs is None:
            limbs = [[(5, 7), (7, 9)], [(6, 8), (8, 10)], [(11, 13), (13, 15)], [(12, 14), (14, 16)], [(5, 6), (5, 11)],
                     [(6, 12)], [(11, 12)], [(1, 2), (2, 3), (3, 4), (1, 0)]]

        self.img_w = img_w
        self.blur_factor = blur_factor
        self.limbs = limbs
        self.squish_channels = squish_channels
        self.json_root = json_root
        self.use_heatmap = use_heatmap
        
    def __scale_keypoints(self, keypoints, original_width, original_height, new_width, new_height):
        xs, ys = keypoints[::3], keypoints[1::3]
        scale_x = new_width / original_width
        scale_y = new_height / original_height
        xs = np.array(xs) * scale_x
        ys = np.array(ys) * scale_y
        coords = np.array(list(zip(xs, ys)))

        return coords
    
    def extract_keypoints_from_dicts(self, dict_list_path, image_id):
    # This list will store the keypoints from the matching dictionaries
        keypoints_list = []
        with open(dict_list_path, 'r') as file:
            dict_list = json.load(file)
        # Iterate over each dictionary in the list
        image_id = str(image_id)
        for dict_item in dict_list:
            # Check if the dictionary's 'image_id' is in the image_ids list
            if dict_item.get('image_id') in [image_id]:
                # Extract the keypoints and add them to the keypoints_list
                keypoints = dict_item.get('keypoints', [])
                keypoints_list.append(keypoints)

        return keypoints_list
    
    def __gaussian_smoothing(self, channel):
        k_size = 1 + int(self.img_w / self.blur_factor) * 2
        return cv2.GaussianBlur(channel, (k_size, k_size), 0, borderType=cv2.BORDER_CONSTANT)

    def __norm_channel(self, channel):
    ...
    def __call__(self, results):
        ...
        video_path = results['frame_dir']
        json_path = self.json_root + video_path.split('/')[-1].split('.')[0] + '.json'
        for frame_idx in frame_indices:
            frame_idx += offset
            keypoints = self.extract_keypoints_from_dicts(json_path, frame_idx)
            if len(keypoints) == 0:
                limb_heatmap = np.zeros((self.img_w, self.img_w, 1), dtype=np.uint8)
                final_out.append(limb_heatmap)
                continue
            limb_heatmaps = []
            for pose in keypoints:
                pose = self.__scale_keypoints(pose, 1920, 1080, self.img_w, self.img_w)
                xs = pose[:, 0] #* (self.img_w - 1)
                ys = pose[:, 1] #* (self.img_w - 1)\
                limb_heatmap = np.zeros((self.img_w, self.img_w), dtype=np.uint8)
                for limb in self.limbs:
                    for edge in limb:
                        try:
                            cv2.line(limb_heatmap, (int(xs[edge[0]]), int(ys[edge[0]])),
                                    (int(xs[edge[1]]), int(ys[edge[1]])), 1, 1)
                            #limb_heatmap = self.__gaussian_smoothing(limb_heatmap.astype(float))
                            #limb_heatmap = self.__norm_channel(limb_heatmap)
                        except Exception:
                            pass
                limb_heatmaps.append(limb_heatmap)
            out = np.dstack(limb_heatmaps)
            out = self.__gaussian_smoothing(out.astype(float))  # Apply Gaussian smoothing once
            if len(out.shape) == 2:
                out = np.expand_dims(out, axis=2)
            out = out.sum(axis=2, keepdims=True)
            final_out.append(out)
        final_out = np.array(final_out)
        results['heatmap'] = final_out
        return results
    ...
    def __call__(self, results):
        ...
        if self.mode == 'directory':
            ...
            if results['frame_inds'].ndim != 1:
                results['frame_inds'] = np.squeeze(results['frame_inds'])
            offset = results.get('offset', 0)
            cap = cv2.VideoCapture(video) if video.endswith('.mp4') else cv2.VideoCapture(video + '.mp4')
        ...
        if self.input_format == 'NCTHW':
            ...
            if 'heatmap' in results:
                if len(heatmap.shape) == 4:
                    heatmap = heatmap.reshape((-1, num_clips, clip_len) + heatmap.shape[1:])  
                    heatmap = np.transpose(heatmap, (0, 1, 5, 2, 3, 4))# N_crops x N_clips x L x H x W x C
...
class ToTensor:
    ...
    def __init__(self, keys, use_heatmap = True):
        self.keys = keys
        self.use_heatmap = use_heatmap

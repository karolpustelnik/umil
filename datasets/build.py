...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def prepare_train_frames(self, idx):
        ...
        if self.use_heatmap:
            aug1['imgs'] = torch.cat((aug1['imgs'], aug1['heatmap'].float()), 1)
        # import pdb;pdb.set_trace()
        if self.repeat > 1:
            aug2 = self.pipeline_(results)
            if self.use_heatmap:
                aug2['imgs'] = torch.cat((aug2['imgs'], aug2['heatmap'].float()), 1)
    ...
    def prepare_test_frames(self, idx):
        ...
        if self.use_heatmap:
            results['imgs'] = torch.cat((results['imgs'].float(), results['heatmap'].float()), 1)
        return results
...
def build_dataloader(logger, config):
    ...
    train_pipeline = [
        ...
        dict(type='RawFrameDecode', mode = config.DATA.FILE_MODE),
        dict(type='Resize', scale=(-1, scale_resize)),
        dict(type='CenterCrop', crop_size=config.DATA.INPUT_SIZE),
        dict(type='Normalize', **img_norm_cfg),
        dict(type='CreateLimbHeatmaps', json_root=config.DATA.KEYPOINTS_ROOT, use_heatmap = config.DATA.USE_HEATMAP),
        dict(type='FormatShape', input_format='NCTHW'),
        dict(type='Collect', keys=['imgs', 'label', 'vid', 'frame_inds', 'total_frames'], meta_keys=[], use_heatmap = config.DATA.USE_HEATMAP),
        dict(type='ToTensor', keys=['imgs', 'label'], use_heatmap = config.DATA.USE_HEATMAP),
    ]
    ...
    train_data = FrameDataset(ann_file=config.DATA.TRAIN_FILE, data_prefix=config.DATA.ROOT,
                              ...
                              use_heatmap = config.DATA.USE_HEATMAP)
    ...
    val_pipeline = [
        ...
        dict(type='CreateLimbHeatmaps', json_root = config.DATA.KEYPOINTS_ROOT, use_heatmap = config.DATA.USE_HEATMAP),
        dict(type='FormatShape', input_format='NCTHW'),
        dict(type='Collect', keys=['imgs', 'label', 'vid'], meta_keys=[], use_heatmap = config.DATA.USE_HEATMAP),
        dict(type='ToTensor', keys=['imgs'], use_heatmap = config.DATA.USE_HEATMAP)
    ]
    ...
    if config.TEST.NUM_CLIP > 1:
        val_pipeline[0] = dict(type='SampleFrames', clip_len=config.DATA.NUM_FRAMES, frame_interval=config.DATA.FRAME_INTERVAL, num_clips=config.DATA.NUM_CLIPS) ###changed

    val_data = FrameDataset(ann_file=config.DATA.VAL_FILE, data_prefix=config.DATA.ROOT, 
                            labels_file=config.DATA.LABEL_LIST, filename_tmpl=config.DATA.FILENAME_TMPL, 
                            pipeline=val_pipeline, use_heatmap = config.DATA.USE_HEATMAP)
    ...
    test_data = RawFramesTestDataset(ann_file=config.DATA.VAL_FILE, data_prefix=config.DATA.ROOT,
                            labels_file=config.DATA.LABEL_LIST, filename_tmpl=config.DATA.FILENAME_TMPL,
                            pipeline=test_pipeline, seg_interval=config.DATA.NUM_FRAMES*config.DATA.FRAME_INTERVAL, use_heatmap = config.DATA.USE_HEATMAP)
    ...
    train_data_test = RawFramesTestDataset(ann_file=config.DATA.TRAIN_FILE, data_prefix=config.DATA.ROOT,
                                     ...
                                     seg_interval=config.DATA.NUM_FRAMES * config.DATA.FRAME_INTERVAL, use_heatmap = config.DATA.USE_HEATMAP)

...
def main(config):
    ...
    for epoch in range(start_epoch, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(epoch, model, optimizer, lr_scheduler, train_loader, text_labels, config, scaler)

        if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
            epoch_saving(config, epoch, model.module, max_auc, optimizer, lr_scheduler,_,_, logger, config.OUTPUT, is_best)

def train_one_epoch(epoch, model, optimizer, lr_scheduler, train_loader, text_labels, config, scaler, data_dict=None):
    model.train()

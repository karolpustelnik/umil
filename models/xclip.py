...
class XCLIP(CLIP):
    ...
    def uda(self, video_feature, text_feature, train_flag):
        ...
        if train_flag:
            ...
            t_fea_nograd = t_fea # was detached before
    ...
    def forward(self, image, text):
        ...
        if self.training:
            ...
            print('shape of v_features_u:', v_features_u.shape)
...
def build_model(state_dict: dict, T=8, droppath=0., use_checkpoint=False, logger=None, prompts_alpha=1e-1, prompts_layers=2, use_cache=True, mit_layers=4, input_channels = 3):
...
def load(model_path, name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", 
         jit=True, T=8, droppath=0., use_checkpoint=False, logger=None, use_cache=True, prompts_alpha=1e-1, prompts_layers=2, mit_layers=1, input_channels = 3
):
